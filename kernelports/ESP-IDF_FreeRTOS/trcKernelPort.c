 ...
 */
portMUX_TYPE xTraceMutex[TRC_CFG_CORE_COUNT] = { [0 ... (TRC_CFG_CORE_COUNT - 1)] = portMUX_INITIALIZER_UNLOCKED };
#endif
...
traceResult xTraceKernelPortInitialize(TraceKernelPortDataBuffer_t* pxBuffer)
{
	...
	// {
	// 	xTraceMutex[i] = (portMUX_TYPE)portMUX_INITIALIZER_UNLOCKED;
	// }
...
{
	portENTER_CRITICAL_SAFE(&xTraceMutex[TRC_CFG_GET_CURRENT_CORE()]);
}

void TRC_KERNEL_PORT_EXIT_CRITICAL_SECTION(void)
{
	portEXIT_CRITICAL_SAFE(&xTraceMutex[TRC_CFG_GET_CURRENT_CORE()]);
}
#endif
